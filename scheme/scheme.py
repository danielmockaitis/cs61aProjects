...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        "*** YOUR CODE HERE ***"
        bullseye = scheme_eval(vals[1] , env)
        env.define(target, bullseye)
        return target
        ...
        body = vals.second
        exp = do_lambda_form(Pair(parameter, body), env)
        env.define(func, exp)
        return func
...
def do_let_form(vals, env):
    ...
    for parts in bindings:
        check_form(parts, 2, 2)
        if not scheme_symbolp(parts.first):
            raise SchemeError
        names = Pair(parts.first, names)
        values = Pair(scheme_eval(parts.second.first, env), values)
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            if clause.second == False or clause.second == nil :
...
def check_formals(formals):
    ...
    symbols = []
    while formals != nil:
        if not scheme_symbolp(formals[0]) or formals[0] in symbols:
            raise SchemeError
        symbols.append(formals[0])
        formals = formals.second
