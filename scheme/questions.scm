...
(define (zip pairs)
  (list (apply-to-all (lambda (x) (car x)) pairs) (apply-to-all (lambda (x) (cadr x)) pairs)) 
  )
...
; expect(define (list-partitions total max-pieces max-value)
(define (len x) (if (eq? x nil) 0 (+ 1 (len (cdr x)))))

(define (list-partitions total max-pieces max-value)
  'YOUR-CODE-HERE
  (define (combine total max) 

    (cond ((eq? max 0) nil)
          ((eq? total 0) (cons nil nil))
          ((< total 0) nil)
          (else (append (apply-to-all (lambda (s) (cons max s)) (combine (- total max) max)) (combine total (- max 1))))
          ))

  (keep-if (lambda (x) (<= (len x) max-pieces)) (combine total max-value))
  )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           'YOUR-CODE-HERE
           expr
           (cons (cons 'lambda (cons (apply-to-all analyze (car (zip values))) (apply-to-all analyze body))) (apply-to-all analyze (cadr (zip values))))
           ))
        (else
         'YOUR-CODE-HERE
