...
def make_tweet_fn(text, time, lat, lon):
    ...
    # returns a function that takes a string argument that is one of the keys above and returns the corresponding value.
    "*** YOUR CODE HERE ***"
    def helper(t):
        if t == 'text':
            return text
...
def extract_words(text):
    ...
    for letter in text:
        if letter not in ascii_letters:
            ...
            wordlist = wordlist + letter
    return wordlist.split()  # You may change/remove this line
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    text = extract_words(tweet_text(tweet))
    count = 0
    avg = 0
    for word in text:
        if has_sentiment(get_word_sentiment(word)):
            avg = avg + sentiment_value(get_word_sentiment(word))
            count += 1
    if count == 0:
        return make_sentiment(None)
    else:
        return make_sentiment(avg/count)
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    #input value = a list of position values that are vertices of its perimeter
    # first vertex is identical to last
    # The centroid of a two-dimensional shape is its center of balance, 
    # defined as the intersection of all straight lines that evenly 
    # divide the shape into equal-area halves.
    # return centroid coordinates and area of the polygon
    # if the area is 0, lat and log of the first position
    #x1y2- y1x2
    cx = 0
    cy = 0
    area = 0
    ...
    while counter < len(polygon) - 1:
        if counter + 1 == len(polygon):
            counter = counter + 1
        else:
            x = latitude(polygon[counter])
            x2 = latitude(polygon[counter + 1])
            y = longitude(polygon[counter])
            y2 = longitude(polygon[counter + 1])
            cx = cx + (x + x2)*(x*y2 - x2*y)
            cy = cy + (y + y2)*(x*y2 - x2*y)
            area = area + (x*y2 - x2*y)
            counter = counter + 1

    area = area / 2
    if area == 0.0: #if area is 0
...
def find_state_center(polygons):
    ...
    # parameter is a list of polygons. 
    # The centroid of a shape is the weighted average of 
    #the centroids of its component polygons, weighted by their area.
    # returns a position
    # find x coordinate of each centroid in polygons
    # divide by area of each coordinate
    # geo_distance(position0, position1):
    centroids = apply_to_all(find_centroid, polygons)
    xnum = 0
    ynum = 0
    denom = 0
    for c in centroids:
        ...
        ynum = ynum + c[1]*c[2]
        denom = denom + c[2]
    return make_position(xnum/denom, ynum/denom)
...
def find_closest_state(locations):
    """ returns the closest state to the given location of a tweet
    >>> ny = make_tweet("welcome to new york", None, 41, -74)
    >>> location = tweet_location(ny)
    >>> find_closest_state(location)
    'NJ'
    """
    closest = 2906 # distance in miles from San Fran to New York
    closest_state = '' #this will be the closest state
    for state_name in us_states: # key is the state name i.e CA or HI
        center = find_state_center(us_states[state_name]) # returns the center of the state
        # center = center of state_name
        # def geo_distance(position0, position1):
        center_of_state = make_position(center[0],center[1]) #uncessary, == [44.56, 35.3333]
        distance = geo_distance(center_of_state,locations) #finds a floating point of the distancef
        if distance < closest:
            closest = distance
...
def group_tweets_by_state(tweets):
    ...
    # return a dictionary with keys that are state-names
    # if state has no tweets, don't include
    # group_by_key function returns dictionary that relates each unique key in [key,value] pairs
    # to a list of all values that appear paired with that key
    temp = []
    for x in tweets: 
        locations = tweet_location(x) 
        closest_state = find_closest_state(locations)
        temp = temp + [[closest_state, x]]
    return group_by_key(temp)
...
def average_sentiments(tweets_by_state):
    ...
    """
    "*** YOUR CODE HERE ***"
    # This function takes the dictionary returned by group_tweets_by_state and 
    ...
    # tweets that have sentiment value in that state.
    # analyze_tweet_sentiment, returns a sentiment object
    sentiments_by_state = {} 
    for state in tweets_by_state:
        ...
        for tweet in tweets_by_state[state]:
            ...
            if has_sentiment(sentiment):
                counter = counter + 1
                average = average + sentiment_value(sentiment)
        if counter > 0:
            average = average / counter
            sentiments_by_state[state] = average
