...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.is_real:
                return
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        bees_in_area = self.place.bees[0:]
        if self.armor - amount <= 0:
            for bee in bees_in_area:
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if not self.is_real:
            ...
            if type(self.place.ant) == BodyguardAnt and self.place.ant not in self.power_ants:
                self.place.ant.damage *= 2
                self.power_ants.append(self.place.ant)
            entrances = self.place.entrance
            while entrances != None:
                if entrances.ant != None and entrances.ant not in self.power_ants:
                    entrances.ant.damage *= 2
                    self.power_ants.append(entrances.ant)
                    if type(entrances.ant) == BodyguardAnt and entrances.ant.ant != None:
                        entrances.ant.ant.damage *= 2
                        self.power_ants.append(entrances.ant.ant)
                entrances = entrances.entrance
            exits = self.place.exit
            while exits != None:
                if exits.ant != None and exits.ant not in self.power_ants:
                    exits.ant.damage *= 2
                    self.power_ants.append(exits.ant)
                    if type(exits.ant) == BodyguardAnt and exits.ant.ant != None:
                        exits.ant.ant.damage *= 2
                        self.power_ants.append(exits.ant.ant)
                exits = exits.exit
